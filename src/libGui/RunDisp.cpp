...
void RunDisp::procNextTask()
{
	...
	for ( const auto &id : m_vCamId ){
		...
		if (hasNewFrm){
			...
			m_dispFrm_h->fn_ = m_camFrm_h->fn_;
		}
	...
	if (hasAny){
		m_dcUI->m_dispRgbFrmQ.wrt( m_dispFrm_h.get() );
	}

	//decide if need to sleep
	if (m_dispFrm_h->fn_ % m_frmFreqToLog == 0) {
		uint32_t dt = timeIntervalMillisec(start);
		dumpLog( "RunDisp::procNextTask(): %s, fn=%ulld, dt=%d", m_threadName.c_str(), m_dispFrm_h->fn_, dt);
	}
...
bool RunDisp::procInit()
{
	...
	m_dispFrm_h.reset( new RgbFrm_h( sz.w, sz.h ) );
...
{
	m_dispFrm_h->dump( folderPath, "dispFrm" );
}
