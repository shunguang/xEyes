...
std::string CapSaveRtspH264::createLaunchStr()
{
	...
	if( boost::iequals( m_camCfg.mp4LocationAndPrefix_, "NULL") ){
		launchStream << "rtspsrc debug=1 user-id=" << m_camCfg.usrId_ << " user-pw=" << m_camCfg.usrPw_ << " location=" << m_camCfg.rtspUrl_ << " ! "
	...
	#	endif
		...
		launchStream << "-e -v rtspsrc debug=1 user-id=" << m_camCfg.usrId_ << " user-pw=" << m_camCfg.usrPw_ << "   location=" << m_camCfg.rtspUrl_ << " ! "
			...
			<< "splitmuxsink name=myMp4Sink location=" << m_camCfg.mp4LocationAndPrefix_ << "%04d.mp4 max-size-time="<< m_camCfg.maxSizeTime_ << " max-size-bytes="<< m_camCfg.maxSizeBytes_ << " max-files=" << m_camCfg.maxFiles_ << " send-keyframe-requests=TRUE";
	}
