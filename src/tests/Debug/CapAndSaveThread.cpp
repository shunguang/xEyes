...
int CapAndSaveThread::main_loop() {
    ...
    launch_stream
    << "-e -v rtspsrc  location= rtsp://192.168.1.19:554/stream0?username=admin&password=E10ADC3949BA59ABBE56E057F20F883E/ ! "
    ...
    << "splitmuxsink name=myMp4Sink location=./frontDoor/test_save_%04d.mp4 max-size-time=9200000000000 max-size-bytes=0 max-files=100 send-keyframe-requests=TRUE";
...
int test_gst_rtsp_rcvH264_dec_and_save(int argc, char** argv) {
    ...
    << "appsink name=myYuvSink tsplit. ! "
    << "queue ! rtph264depay ! h264parse ! "
    << "splitmuxsink name=myMp4Sink location=./frontDoor/test_save_%04d.mp4 max-size-time=8200000000000 max-size-bytes=0 max-files=100 send-keyframe-requests=TRUE";
